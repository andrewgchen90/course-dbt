...
SELECT s.user_id,
    ...
    sum(case when s.num_added_to_cart > 0 then s.sessions_id else null end) as lifetime_sessions_with_add_to_cart,
    sum(case when s.num_checkouts > 0 then s.sessions_id else null end) as lifetime_sessions_with_check_out
