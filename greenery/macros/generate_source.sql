...
{% set tables = dbt_utils.get_relations_by_pattern(
    database=database_name, schema_pattern=schema_name, table_pattern="%"
) %} {% set table_list = tables | map(attribute="identifier") %} {{ return(table_list | sort) }}
...
{% set joined = sources_yaml | join("\n") %} {{ log(joined, info=True) }} {% do return(joined) %}
